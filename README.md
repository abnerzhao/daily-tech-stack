...
流程：GitHub Actions 每天北京时间 10:00 抓取六个来源的 Top 10 并提交 `index.html`；Vercel 监听 `main` 的新提交并自动部署。无需远程服务器、cron 或 Vercel Token。
...
4. 在 GitHub 的 `Actions > Update daily issue` 中点击 `Run workflow`，可立即生成首份自动日报；可选填写 `date`（`YYYY-MM-DD`）重跑指定日期，留空则使用当天；之后会按计划每日更新。
